...
public class ItemToTwoItemRecipeCategory<R extends IRRecipe> extends AbstractIRRecipeCategory<R> {

	public ItemToTwoItemRecipeCategory(RecipeType<R> recipeType, Component title) {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, R recipe, IFocusGroup focuses) {
		...
		addItem(builder, RecipeIngredientRole.OUTPUT, 75, 15, getOutput(recipe, 1), JEIDrawables.OUTPUT_SLOT);
	}
	...
	public void draw(R recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
		...
		for(int i = 0; i < 2; ++i) {
			...
			if(chance < 1) {
				...
				guiGraphics.drawString(font, chanceComponent, 56 + i * 28 - font.width(chanceComponent) / 2, 37, 0xFF808080, false);
			}
		}
