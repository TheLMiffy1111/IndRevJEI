...
public class FluidToItemRecipeCategory<R extends IRRecipe> extends AbstractIRRecipeCategory<R> {
	...
	public void createRecipeExtras(IRecipeExtrasBuilder builder, R recipe, IFocusGroup focuses) {
		builder.addDrawable(ProgressBarDrawable.right(recipe), 19, 14);
	}
	...
	public void draw(R recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
		...
		if(chance < 1) {
			...
			guiGraphics.drawString(font, chanceComponent, 54 - font.width(chanceComponent) / 2, 37, 0xFF808080, false);
		}
